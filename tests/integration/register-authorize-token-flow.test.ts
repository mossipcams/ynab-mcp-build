...
describe("register-authorize-token integration", () => {
  it("composes registration, authorization, and token exchange into a usable OAuth flow", async () => {
    ...
    const { registration, response: registrationResponse } =
      await registerClient(env);
    const {
      code,
      location,
      response: authorizeResponse,
    } = await authorizeClient(env, registration.client_id);
    const { payload: tokenPayload, response: tokenResponse } =
      await exchangeCode(env, registration.client_id, code ?? "");
