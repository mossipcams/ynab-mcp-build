...
describe("oauth http authorize", () => {
  ...
  function createAccessDiscoveryResponse() {
    return {
      ...
      token_endpoint:
        "https://access-team.example.com/cdn-cgi/access/sso/oidc/token",
    };
  }
  ...
  it("authorizes a registered MCP client and preserves client state", async () => {
    ...
    const { code, location, response } = await authorizeClient(
      env,
  ...
  it("uses explicit Access OIDC endpoint overrides without discovery", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (request.url.endsWith("/.well-known/openid-configuration")) {
          throw new Error(
            "Discovery should not be requested when endpoint overrides are configured.",
          );
        }
    ...
    expect(response.status).toBe(302);
    expect(redirect.origin + redirect.pathname).toBe(
      "https://access.example.com/authorize",
    );
  });
  ...
  it("exchanges an Access OIDC callback for a local MCP authorization code", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          request.url === "https://access-team.example.com/cdn-cgi/access/certs"
        ) {
    ...
    const { registration } = await registerClient(env);
    const authorizeResponse = await requestAuthorization(
      env,
      registration.client_id,
    );
    ...
    const response = await fetchWorker(
      new Request(
        `${MCP_ORIGIN}/oidc/callback?code=access-code-1&state=${encodeURIComponent(state)}`,
      ),
      env,
    );
    const location = response.headers.get("location");
  ...
  it("rejects Access OIDC callbacks when the upstream token exchange fails", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          ...
          throw new Error(
            "JWKS should not be fetched when token exchange fails.",
          );
        }
    ...
    const response = await fetchWorker(
      new Request(
        `${MCP_ORIGIN}/oidc/callback?code=bad-code&state=${encodeURIComponent(accessRedirect.searchParams.get("state") ?? "")}`,
      ),
      env,
    );
    const payload = (await response.json()) as {
    ...
    expect(requestedUrls).not.toContain(
      "https://access-team.example.com/cdn-cgi/access/certs",
    );
  });
  ...
  it("rejects replayed Access OIDC callback state", async () => {
    ...
    const payload = (await replayResponse.json()) as {
      error: string;
      error_description: string;
    };
  ...
  it("rejects scopes that this MCP server does not grant", async () => {
    ...
    expect(payload.error_description).toBe(
      "Requested scope is not supported: admin",
    );
  });
