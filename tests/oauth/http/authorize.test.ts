...
describe("oauth http authorize", () => {
  ...
  function createAccessOidcEnv() {
    return createOAuthEnv({
      ...
      ACCESS_TEAM_DOMAIN: "mossipcams.cloudflareaccess.com"
  ...
  function createAccessDiscoveryResponse() {
    return {
      authorization_endpoint: "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/sso/oidc/authorize",
      jwks_uri: "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/certs",
      token_endpoint: "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/sso/oidc/token"
    };
  }
  ...
  it("redirects authorization requests through Cloudflare Access OIDC when configured", async () => {
    ...
    expect(redirect.origin + redirect.pathname).toBe(
      "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/sso/oidc/authorize"
    );
  ...
  it("exchanges an Access OIDC callback for a local MCP authorization code", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (request.url === "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/sso/oidc/token") {
      ...
      if (request.url === "https://mossipcams.cloudflareaccess.com/cdn-cgi/access/certs") {
  ...
  it("rejects Access OIDC callbacks when the upstream token exchange fails", async () => {
    ...
    expect(requestedUrls).toContain("https://mossipcams.cloudflareaccess.com/cdn-cgi/access/sso/oidc/token");
    expect(requestedUrls).not.toContain("https://mossipcams.cloudflareaccess.com/cdn-cgi/access/certs");
  });
