...
export function createReadModelFreshness(database: D1Database, options: {
  now: () => string;
  staleAfterMinutes: number;
}) {
  return {
    async getFreshness(planId: string, requiredEndpoints: readonly string[]): Promise<EndpointFreshness> {
      ...
      const missingEndpoint = requiredEndpoints.find((endpoint) => !rowsByEndpoint.has(endpoint));
      ...
      if (unhealthy) {
        return {
          ...
          stale: true,
          warning: unhealthy.last_error ?? `Required endpoint ${unhealthy.endpoint} is unhealthy.`
        };
      }

      const staleBefore = subtractMinutes(options.now(), options.staleAfterMinutes);
      ...
      return {
        ...
        last_synced_at: oldestLastSyncedAt,
        stale,
        warning: stale ? "Data is stale relative to the configured freshness window." : null
      };
    }
  };
}
