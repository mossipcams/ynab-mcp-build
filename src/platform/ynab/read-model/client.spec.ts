...
class FakeD1Database {
  ...
  select(sql: string, params: unknown[]) {
    ...
    if (sql.includes("COUNT(*)")) {
      return [
        { count: rows.filter((row) => this.matches(row, sql, params)).length },
      ];
    }
...
describe("YNAB read-model client", () => {
  ...
  it("serves core YNAB client reads from D1 rows without HTTP access", async () => {
    ...
    await expect(
      client.listTransactionsByPayee("plan-1", "payee-1"),
    ).resolves.toMatchObject([
      {
        id: "txn-1",
        amount: -4000,
  ...
  it("maps the full D1-backed client surface with bound plan-scoped queries", async () => {
    ...
    await expect(
      client.listTransactionsByAccount("plan-1", "account-1"),
    ).resolves.toMatchObject([
      {
        id: "txn-1",
        approved: null,
    ...
    await expect(
      client.listTransactionsByCategory("plan-1", "category-1"),
    ).resolves.toHaveLength(1);
    await expect(
    ...
    await expect(
      client.getPayeeLocation("plan-1", "loc-1"),
    ).resolves.toMatchObject({
      id: "loc-1",
      latitude: 41.1,
    });
  ...
  it("rejects missing detail rows with read-model specific errors", async () => {
    const client = createYnabReadModelClient(
      new FakeD1Database() as unknown as D1Database,
    );

    await expect(client.getUser()).rejects.toThrow(
      "No synced YNAB user is available.",
    );
    await expect(client.getPlan("missing-plan")).rejects.toThrow(
      "YNAB plan missing-plan was not found",
    );
    await expect(
      client.getCategory("plan-1", "missing-category"),
    ).rejects.toThrow("YNAB category missing-category was not found");
    await expect(
      client.getMonthCategory("plan-1", "2026-04-01", "missing-category"),
    ).rejects.toThrow("YNAB month category missing-category was not found");
    await expect(client.getPlanSettings("plan-1")).rejects.toThrow(
      "YNAB plan settings for plan-1 were not found",
    );
    await expect(client.getPlanMonth("plan-1", "2026-04-01")).rejects.toThrow(
      "YNAB month 2026-04-01 was not found",
    );
    await expect(
      client.getAccount("plan-1", "missing-account"),
    ).rejects.toThrow("YNAB account missing-account was not found");
    await expect(
      client.getTransaction("plan-1", "missing-transaction"),
    ).rejects.toThrow("YNAB transaction missing-transaction was not found");
    await expect(
      client.getScheduledTransaction("plan-1", "missing-scheduled"),
    ).rejects.toThrow(
      "YNAB scheduled transaction missing-scheduled was not found",
    );
    await expect(client.getPayee("plan-1", "missing-payee")).rejects.toThrow(
      "YNAB payee missing-payee was not found",
    );
    await expect(
      client.getPayeeLocation("plan-1", "missing-location"),
    ).rejects.toThrow("YNAB payee location missing-location was not found");
  });
});
