...
describe("worker entrypoint", () => {
  const publicBaseUrl = "https://ynab-mcp-build.mossipcams.workers.dev";
  ...
  it("serves OAuth metadata with the Cloudflare provider bindings", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      authorization_endpoint: "https://ynab-mcp-build.mossipcams.workers.dev/authorize",
      registration_endpoint: "https://ynab-mcp-build.mossipcams.workers.dev/register",
      token_endpoint: "https://ynab-mcp-build.mossipcams.workers.dev/token"
    });
  });
  ...
  it("requires OAuth access tokens for MCP when OAuth mode is enabled", async () => {
    ...
    expect(response.headers.get("www-authenticate")).toContain(
      'resource_metadata="https://ynab-mcp-build.mossipcams.workers.dev/.well-known/oauth-protected-resource/mcp"'
    );
  });
});
