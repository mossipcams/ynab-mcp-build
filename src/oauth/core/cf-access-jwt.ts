...
const JsonWebKeyWithKidSchema = z
  .object({
    kid: z.string().optional(),
  })
  .passthrough() as unknown as z.ZodType<JsonWebKey & { kid?: string }>;

const CfAccessJwtHeaderSchema = z
  .object({
    alg: z.string(),
    kid: z.string().optional(),
  })
...
const CfAccessJwtPayloadSchema = z
  .object({
...
export async function verifyCfAccessJwt(
  ...
  const header = await parseJsonFromBase64url<CfAccessJwtHeader>(
    headerB64,
    CfAccessJwtHeaderSchema,
  );
  const payload = await parseJsonFromBase64url<CfAccessJwtPayload>(
    payloadB64,
    CfAccessJwtPayloadSchema,
  );
  ...
  if (payload.iss !== normalizedDomain) {
    throw new Error(
      `CF Access token issuer mismatch: expected ${normalizedDomain}, got ${payload.iss}`,
    );
  }
  ...
  const signatureBytes = base64urlDecode(
    signatureB64,
  ) as Uint8Array<ArrayBuffer>;
