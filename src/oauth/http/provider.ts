...
function createScopedApiHandler(apiHandler: HandlerWithFetch) {
  return {
    fetch(request, env, executionContext) {
      if (
        !(executionContext as OAuthExecutionContext).props?.scopes?.includes(
          "mcp",
        )
      ) {
...
function createOAuthProviderOptions(
...
export function createOAuthProvider(apiHandler: ExportedHandler<Env>) {
  return new OAuthProvider<Env>(
    createOAuthProviderOptions(apiHandler as HandlerWithFetch),
  );
}
