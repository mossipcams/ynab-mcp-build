...
export async function listAccounts(
  ...
  if (
    !shouldPaginateEntries(accounts, input) &&
  ...
  if (!shouldPaginateEntries(accounts, input)) {
    return {
      accounts: accounts.map((account) =>
        projectRecord(account, accountFields, input),
      ),
      account_count: accounts.length,
    };
  }
  ...
  return {
    accounts: pagedAccounts.entries.map((account) =>
      projectRecord(account, accountFields, input),
    ),
    account_count: accounts.length,
    ...
    account: compactObject({
      ...
      balance:
        account.balance == null
          ? undefined
          : formatAmountMilliunits(account.balance),
    }),
  };
}
