import type { YnabAccountSummary, YnabClient, YnabPlanMonthDetail, YnabTransaction } from "../../platform/ynab/client.js";
...
type FinancialHealthCategory = NonNullable<YnabPlanMonthDetail["categories"]>[number];
...
function previousMonths(latestMonth: string, count: number) {
  return Array.from({ length: count }, (_, index) => addMonths(latestMonth, -(count - index)));
}
...
function sortRollupsByAmount<T extends { amountMilliunits: number; name: string }>(entries: T[]) {
  return entries
    .slice()
    .sort((left, right) => right.amountMilliunits - left.amountMilliunits || left.name.localeCompare(right.name));
}

async function resolveMonth(ynabClient: YnabClient, planId: string, month?: string) {
  if (isExplicitMonth(month)) {
    return month;
  }

  const months = await ynabClient.listPlanMonths(planId);
  const resolvedMonth = months.find((entry) => !entry.deleted)?.month ?? months[0]?.month;
...
async function getRangeContext(ynabClient: YnabClient, input: FinancialHealthRangeInput) {
  const planId = await resolvePlanId(ynabClient, input.planId);
...
function toTopRollups(entries: Array<{ id?: string; name: string; amountMilliunits: number }>, topN: number): DisplayRollup[] {
  return entries
    .slice()
    .sort((left, right) => right.amountMilliunits - left.amountMilliunits || left.name.localeCompare(right.name))
    .slice(0, topN)
...
function summarizeBudgetHealthMonth(monthDetail: YnabPlanMonthDetail, topN: number) {
  ...
  return {
    ...
    ...buildAssignedSpentSummary(monthDetail.budgeted ?? 0, toSpentMilliunits(monthDetail.activity ?? 0)),
    ...
    top_underfunded_categories: underfundedCategories.slice(0, topN).map((category) =>
      compactObject({
        id: category.id,
        name: category.name,
        category_group_name: category.categoryGroupName,
...
function isNonTransferMonthTransaction(transaction: YnabTransaction, month: string) {
  return !transaction.deleted && !transaction.transferAccountId && transaction.date.startsWith(month.slice(0, 7));
}

function isNonTransferRangeTransaction(transaction: YnabTransaction, fromMonth: string, toMonth: string) {
...
function buildCleanupCounts(transactions: YnabTransaction[]) {
  ...
  return {
    uncategorizedTransactionCount: visibleTransactions.filter((transaction) => !transaction.categoryId).length,
    unapprovedTransactionCount: visibleTransactions.filter((transaction) => !transaction.approved).length,
    unclearedTransactionCount: visibleTransactions.filter((transaction) => transaction.cleared === "uncleared").length
  };
}
...
function buildHealthRisks(input: {
  ...
  const risks: Array<{ code: string; severity: "high" | "medium"; penalty: number }> = [];

  if (input.readyToAssign < 0) {
    risks.push({ code: "negative_ready_to_assign", severity: "high", penalty: 20 });
  }
...
function buildSpendingAnomalies(
  ...
  const baselineSpentLookups = baselineMonthDetails.map((monthDetail) =>
    new Map(
      (monthDetail.categories ?? [])
        .filter((category) => !category.deleted && !category.hidden)
        .map((category) => [category.id, toSpentFromActivity(category.activity)] as const)
    )
  );
  ...
  return latestCategories
    ...
    .map((category) => {
      ...
      const baselineValues = baselineSpentLookups
        .map((lookup) => lookup.get(category.id) ?? 0);
      const baselineAverage = baselineValues.length
        ? baselineValues.reduce((sum, value) => sum + value, 0) / baselineValues.length
        : 0;
      const increase = latestSpent - baselineAverage;
      const increasePercent = baselineAverage > 0 ? (increase / baselineAverage) * 100 : undefined;
    ...
    .filter((entry) => (entry.baselineAverage > 0
      ? entry.latestSpent >= entry.baselineAverage * options.thresholdMultiplier
      : entry.latestSpent > 0))
    .sort((left, right) => right.increase - left.increase || left.category_name.localeCompare(right.category_name))
...
function toMonthSpendingRollups(transactions: YnabTransaction[], topN: number) {
  const rollups = new Map<string, { id?: string; name: string; amountMilliunits: number; transactionCount: number }>();
...
export async function getFinancialSnapshot(ynabClient: YnabClient, input: FinancialHealthInput) {
  ...
  return {
    ...
    debt: formatMilliunits(snapshot.negativeAccounts.reduce((sum, account) => sum + Math.abs(account.balance ?? 0), 0)),
    ready_to_assign: formatMilliunits(monthDetail.toBeBudgeted ?? 0),
...
export async function getFinancialHealthCheck(ynabClient: YnabClient, input: FinancialHealthInput) {
  ...
  const risks = buildHealthRisks({
    ...
    cleanupCount:
      cleanup.uncategorizedTransactionCount + cleanup.unapprovedTransactionCount + cleanup.unclearedTransactionCount,
  ...
  return {
    ...
    top_risks: risks.slice(0, topN).map((entry) => compactObject({ code: entry.code, severity: entry.severity }))
  };
}
...
export async function getCashFlowSummary(ynabClient: YnabClient, input: FinancialHealthRangeInput) {
  const { fromMonth, toMonth, months, transactions } = await getRangeContext(ynabClient, input);
  ...
  const spentMilliunits = months.reduce((sum, month) => sum + toSpentFromActivity(month.activity), 0);
  ...
  return {
    ...
    periods: months.map((month) => {
      ...
      return {
        ...
        ...buildAssignedSpentSummary(month.budgeted, toSpentFromActivity(month.activity))
      };
    })
  };
}
...
export async function getSpendingSummary(ynabClient: YnabClient, input: FinancialHealthRangeInput) {
  ...
  const categoryGroupRollups = new Map<string, { name: string; amountMilliunits: number; transactionCount: number }>();
  ...
  return {
    ...
    average_transaction: formatMilliunits(
      spendingTransactions.length ? Math.round(spentMilliunits / spendingTransactions.length) : 0
    ),
...
export async function getCategoryTrendSummary(ynabClient: YnabClient, input: CategoryTrendSummaryInput) {
  ...
  const monthDetails = await Promise.all(months.map((month) => ynabClient.getPlanMonth(planId, month)));
  const periods = monthDetails.map((monthDetail) => {
    const matchingCategories = (monthDetail.categories ?? []).filter((category) => {
    ...
    const assignedMilliunits = matchingCategories.reduce((sum, category) => sum + (category.budgeted ?? 0), 0);
    const spentMilliunits = matchingCategories.reduce((sum, category) => sum + toSpentFromActivity(category.activity), 0);
...
export async function getCashRunway(ynabClient: YnabClient, input: { planId?: string; asOfMonth: string; monthsBack?: number }) {
  const planId = await resolvePlanId(ynabClient, input.planId);
  ...
  const averageMonthlySpending = consideredMonths.length
    ? consideredMonths.reduce((sum, month) => sum + toSpentFromActivity(month.activity), 0) / consideredMonths.length
    : 0;
  ...
  const scheduledNetNext30d = scheduledTransactions
    ...
    .reduce((sum, transaction) => sum + transaction.amount, 0);
  const runwayDays = averageDailyOutflow === 0 ? undefined : liquidCash / averageDailyOutflow;
  const status = runwayDays == null ? "no_outflows" : runwayDays >= 90 ? "stable" : runwayDays >= 30 ? "watch" : "urgent";
...
export async function getUpcomingObligations(
  ...
  const scheduledTransactions = (await ynabClient.listScheduledTransactions(planId))
    ...
    .filter((transaction) => transaction.days_until_due >= 0 && transaction.days_until_due <= 30)
    .sort((left, right) => left.days_until_due - right.days_until_due || Math.abs(right.amount) - Math.abs(left.amount));
    ...
    [7, 14, 30].map((windowDays) => {
      ...
      return [
        `${windowDays}d`,
        {
          ...
          obligation_count: windowTransactions.filter((transaction) => transaction.amount < 0).length,
          expected_inflow_count: windowTransactions.filter((transaction) => transaction.amount > 0).length
        }
      ];
    })
  );
  ...
  return {
    ...
    windows,
    top_due: scheduledTransactions.slice(0, topN).map((transaction) => compactObject({
      id: transaction.id,
...
export async function getIncomeSummary(ynabClient: YnabClient, input: FinancialHealthRangeInput) {
  ...
  const averageIncome = incomeValues.length ? incomeTotal / incomeValues.length : 0;
  ...
  const medianIncome = sortedIncome.length === 0
    ...
    : sortedIncome.length % 2 === 1
      ...
      : (((sortedIncome[(sortedIncome.length / 2) - 1] ?? 0) + (sortedIncome[sortedIncome.length / 2] ?? 0)) / 2);
...
function detectCadence(dates: string[]) {
  ...
  const intervals = sortedDates.slice(1).map((date, index) => daysUntil(sortedDates[index]!, date));
...
export async function getEmergencyFundCoverage(
  ...
  const scheduledNetNext30d = scheduledTransactions
    ...
    .reduce((sum, transaction) => sum + transaction.amount, 0);
  const coverageMonths = averageMonthlySpending === 0 ? undefined : liquidCash / averageMonthlySpending;
  const status = coverageMonths == null
    ? "no_spending"
    : coverageMonths >= 6
      ? "strong"
      : coverageMonths >= 3
  ...
  return compactObject({
    as_of_month: input.asOfMonth,
    liquid_cash: formatMilliunits(liquidCash),
    average_monthly_spending: formatMilliunits(Math.round(averageMonthlySpending)),
    scheduled_net_next_30d: formatMilliunits(scheduledNetNext30d),
...
export async function getGoalProgressSummary(ynabClient: YnabClient, input: FinancialHealthInput) {
  ...
  const underfundedGoals = goalCategories
    ...
    .sort((left, right) => (right.goalUnderFunded ?? 0) - (left.goalUnderFunded ?? 0));

  return {
    month: monthDetail.month,
    goal_count: goalCategories.length,
    underfunded_total: formatMilliunits(underfundedGoals.reduce((sum, category) => sum + (category.goalUnderFunded ?? 0), 0)),
    on_track_count: goalCategories.filter((category) => (category.goalUnderFunded ?? 0) === 0).length,
...
export async function getCleanupSummary(ynabClient: YnabClient, input: FinancialHealthInput) {
  ...
  const uncategorizedTransactions = monthTransactions.filter((transaction) => !transaction.categoryId);
  const unapprovedTransactions = monthTransactions.filter((transaction) => !transaction.approved);
  const unclearedTransactions = monthTransactions.filter((transaction) => transaction.cleared === "uncleared");
  ...
  const hiddenProblemCategories = (monthDetail.categories ?? [])
    .filter((category) => !category.deleted && category.hidden && (category.balance < 0 || (category.goalUnderFunded ?? 0) > 0));
...
export async function getNetWorthTrajectory(ynabClient: YnabClient, input: { planId?: string; fromMonth?: string; toMonth?: string }) {
  const planId = await resolvePlanId(ynabClient, input.planId);
  const { fromMonth, toMonth } = await resolveMonthRange(ynabClient, planId, input.fromMonth, input.toMonth);
  ...
  const months = listMonthsInRange(fromMonth, toMonth);
  const balancesByMonth = reconstructHistoricalBalances(accounts.filter((account) => !account.deleted), transactions, months);
  const monthSummaries = months.map((month) => summarizeBalances(month, balancesByMonth.get(month) ?? new Map(), accounts));
  ...
  return {
    ...
    change_net_worth: formatMilliunits(
      Number.parseFloat(endSummary?.net_worth ?? "0") * 1000 - Number.parseFloat(startSummary?.net_worth ?? "0") * 1000
    ),
