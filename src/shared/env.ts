...
export function resolveAppEnv(
  ...
  const hasAccessOidcValues =
    accessOidcRequiredValueCount > 0 || accessOidcOverrideValueCount > 0;
  ...
  const resolvedEnv = compact({
    ...(accessOidcRequiredValueCount === 3
      ? {
          accessOidc: {
            ...
            discoveryUrl: getAccessOidcDiscoveryUrl(
              accessTeamDomain!,
              accessOidcValues.clientId!,
            ),
    ...
    publicUrl:
      runtimeEnv?.MCP_PUBLIC_URL ??
      (oauthEnabled ? undefined : derivedPublicUrl),
    ynabApiBaseUrl:
      runtimeEnv?.YNAB_API_BASE_URL ?? DEFAULT_APP_ENV.ynabApiBaseUrl,
    ynabAccessToken:
      runtimeEnv?.YNAB_ACCESS_TOKEN ?? runtimeEnv?.YNAB_API_TOKEN,
  ...
  if (
    resolvedEnv.oauthEnabled &&
    !resolvedEnv.oauthStateNamespace &&
    !resolvedEnv.oauthKvNamespace
  ) {
    throw new Error(
